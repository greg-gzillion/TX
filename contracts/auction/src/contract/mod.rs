...
use crate::msg::{AuctionResponse, BidResponse, ExecuteMsg, InstantiateMsg, QueryMsg};
use crate::state::{Auction, Bid, Config, AUCTIONS, AUCTION_COUNT, BIDS, BID_COUNT, CONFIG};
use cosmwasm_std::{
    coins, entry_point, to_json_binary, BankMsg, Binary, Deps, DepsMut, Env, MessageInfo, Response,
    StdResult, Uint128,
};
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::CloseAuction { auction_id } => {
            execute_close_auction(deps, env, info, auction_id)
        }
        ExecuteMsg::ClaimWinnings { auction_id } => {
            execute_claim_winnings(deps, env, info, auction_id)
        }
    }
}
...
fn execute_create_auction(
    ...
    auction_count = auction_count.checked_add(1).unwrap_or(0);
