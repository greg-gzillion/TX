...
export default function HomePage() {
  ...
  useEffect(() => {
    const fetchFeatured = async () => {
      try {
        const response = await fetch('https://phoenix-api-756y.onrender.com/api/auctions?featured=true');
        const data = await response.json();
        if (data.auctions) {
          ...
          {featuredAuctions.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {featuredAuctions.map((auction) => (
                <div key={auction.id} className="border rounded-lg p-3 bg-white">
                  ...
                  <p className="text-xs text-gray-600">{auction.weight} {auction.weightUnit}</p>
                  <p className="text-amber-700 font-bold mt-1">${auction.currentBid?.toFixed(0) || auction.startingPrice?.toFixed(0)}</p>
                </div>
              ))}
            </div>
          ) : (
            <div className="text-center py-8 bg-gray-50 rounded-lg">
              ...
              <Link href="/auctions/create" className="text-amber-600 text-sm mt-2 inline-block">
