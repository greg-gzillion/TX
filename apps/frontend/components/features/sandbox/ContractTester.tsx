...
export function ContractTester({ wallet }: { wallet: any }) {
  ...
  const handleTest = async () => {
    ...
    setTimeout(() => {
      setResult(
        `✅ Test successful! Function "${selectedFunction}" executed with params: ${params}`,
      );
      setLoading(false);
    }, 1500);
  };
    ...
    <div>
      ...
      <div className="bg-purple-100 border border-purple-300 rounded-lg p-3 mb-6">
        <p className="text-purple-800 text-sm flex items-center">
          ...
          <strong>SANDBOX MODE:</strong> All contract interactions are
          simulated.
          <a href="/auctions" className="underline ml-1 font-bold">
            View real auctions →
          </a>
        </p>
      </div>
      ...
      <p className="text-gray-600 mb-6">
        Test contract functions with mock data. Real contract deployment coming
        March 6.
      </p>
      ...
      {!wallet ? (
        <div className="bg-yellow-50 p-4 rounded-lg">
          <p className="text-yellow-700">
            Please select a wallet above to test contracts.
          </p>
        </div>
      ) : (
        <div className="space-y-6">
          ...
          <div className="p-4 bg-blue-50 rounded-lg">
            <p className="text-sm text-blue-700">
              ⚙️ This is a mock contract tester. Real contract integration will
              be available March 6. Your selected wallet:{" "}
