...
export default function PuritySelector({ metalType, value, onChange }: PuritySelectorProps) {
  ...
  // Get comprehensive purity options based on metal type
    ...
    switch(metalType) {
      case 'Gold':
        return [
          ...
          { value: 0.750, label: '.750', desc: '18 Karat (jewelry)' },
          { value: 0.585, label: '.585', desc: '14 Karat (jewelry)' },
          { value: 0.417, label: '.417', desc: '10 Karat (jewelry)' },
      ...
      case 'Silver':
        return [
          ...
          { value: 0.925, label: '.925', desc: 'Sterling Silver (jewelry, tableware)' },
      ...
      case 'Platinum':
        return [
          ...
          { value: 0.999, label: '.999', desc: 'Three Nines Fine' },
          { value: 0.950, label: '.950', desc: '95% Platinum (jewelry, US Eagles)' },
          { value: 0.900, label: '.900', desc: '90% Platinum (older coins)' },
          { value: 0.850, label: '.850', desc: '85% Platinum (jewelry)' },
        ];
      ...
      case 'Copper':
        return [
          ...
          { value: 0.750, label: '.750', desc: '75% Copper/25% Nickel (wartime nickels)' },
          { value: 0.600, label: '.600', desc: '60% Copper/40% Silver (clad proofs)' },
          { value: 0.025, label: '.025', desc: '2.5% Copper/97.5% Zinc (modern pennies 1982-present)' },
          { value: 0.000, label: 'Zinc', desc: 'Zinc core with copper plating (modern cents)' },
        ];
      
      default:
        return [
          { value: 0.999, label: '.999', desc: 'Fine' },
          { value: 0.925, label: '.925', desc: 'Sterling' },
          { value: 0.900, label: '.900', desc: '90%' },
          { value: 0.750, label: '.750', desc: '75%' },
        ];
    }
  };
  ...
  return (
    <div className="space-y-4">
      <label className="block text-sm font-medium text-gray-700">
        Metal Purity / Composition ({metalType})
      </label>
        ...
        {isCustom && (
          <div className="mt-2 p-3 bg-gray-50 rounded-lg">
            ...
            <p className="text-xs text-gray-500 mt-1">
              = {(value * 100).toFixed(2)}% primary metal
            </p>
          </div>
        )}
      </div>
