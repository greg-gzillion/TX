  ...
  return (
    <div className="border rounded-lg p-4 hover:shadow-lg transition">
      ...
      <div className="mt-2 space-y-1 text-sm text-gray-600">
        <p>
          Weight: {metadata?.item?.weight} {metadata?.item?.weightUnit}
        </p>
        <p>
          Purity:{" "}
          {metadata?.item?.purity
            ? (parseFloat(metadata.item.purity) * 100).toFixed(1)
            : "N/A"}
          %
        </p>
        ...
        {metadata?.item?.serialNumber && (
          <p>Serial: {metadata.item.serialNumber}</p>
        )}
      </div>
      ...
      <div className="mt-4 space-y-2">
        ...
        {auction.reserve_price && (
          <div className="text-xs text-gray-400 flex justify-between">
            <span>
              Reserve: ${formatTestUsd(auction.reserve_price)} TESTUSD
            </span>
            {auction.current_bid &&
              parseInt(auction.current_bid) >=
                parseInt(auction.reserve_price) && (
