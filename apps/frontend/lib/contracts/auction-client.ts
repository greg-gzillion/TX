...
export class MockAuctionClient {
  ...
  async placeBid(auctionId: number, bidAmount: string): Promise<boolean> {
    ...
    const currentPrice =
      auctions[auctionIndex].current_bid?.amount ||
      auctions[auctionIndex].starting_price;
  ...
  async endAuction(
    ...
    if (
      auctions[auctionIndex].seller !==
      "testcore1mj58cdfrkc8uyunw2rna3wvkatdjfhd6f6spqd"
    ) {
  ...
  async releaseToSeller(auctionId: number): Promise<boolean> {
    ...
    if (
      auctions[auctionIndex].winner &&
      auctions[auctionIndex].winner !== "No bids placed"
    ) {
      console.log(
        `💰 [MOCK] Transferring ${auctions[auctionIndex].winning_amount} TEST from ${auctions[auctionIndex].winner} to ${auctions[auctionIndex].seller}`,
      );
    } else {
  ...
  async getUserAuctions(userAddress: string): Promise<Auction[]> {
    ...
    return auctions.filter(
      (a) => a.seller === userAddress || a.current_bid?.bidder === userAddress,
    );
  }
